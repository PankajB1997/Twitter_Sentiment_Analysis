...
with open(CSV_SAVEPATH, "a") as csvfile:
    ...
    for path in PKL_PATHS:
        for pickle_filepath in os.listdir(path):
            ...
            pd.DataFrame(tweets_current_pkl).to_csv(csvfile, sep=",", header=False, index=False, encoding="ISO-8859-1")
            size += len(tweets_current_pkl)

    for path in CSV_PATHS:
        tweets_current_csv = pd.read_csv(path, header=None, index_col=None)
        tweets_current_csv.to_csv(csvfile, sep=",", header=False, index=False, encoding="ISO-8859-1")
